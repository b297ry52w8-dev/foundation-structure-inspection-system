(function(){
  ...
  const reports=Array.from({length:9},(_,i)=>({id:'BG2026'+String(i+1).padStart(6,'0'),taskCount:(i%4)+1,entrust:entrusts[i%entrusts.length].id,created:`2026-08-${String(12+i).padStart(2,'0')} 10:00`,author:['张工','李工','王工'][i%3],status:['编制中','编制中','已完成','已作废'][i%4],approval:['待发起','已驳回','审核通过','审核通过'][i%4],version:i%3+1,attachment:i%4===0?'':'检测报告.pdf'}));
  const businessTypes=[
